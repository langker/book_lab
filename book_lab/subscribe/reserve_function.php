...
if(!function_exists('createTable'))
{
	...
	function createTable($nameList, $table, $date, $isMeeting)
	{
		...
		$currentDay = date("n", $time)."月".date("j", $time)."日"."($weekDay)";//~获得当前时间
		//生成表格主体
		if(!$isMeeting)
		{
			...
			for($i = 0; $i < 5; $i++)
			{
				...
				foreach ($table[$i] as $reserveList)
				{
					if($reserveList)
					{
						$query_user = $conn_db->select("user", "*", "user.user_id='$reserveList'");
					...
					{
					echo "<th><button data-tableRow=\"$i\" data-tableCul=\"".$currentCou."\" data-modal=\"modal-15\" class=\"md-trigger button medium blue\">预定</button></th>";
					}
					$currentCou++;	//再加上去的
				}
				echo "</tr>\n";
...
if(!function_exists("getTableInfo"))
{
	function &getTableInfo(&$db, $date, $labType, $labBuilding)
	{
		...
		for($i = 0; $i < $nameSize; $i++)
		{
			...
			$_reserveList = $db->select("lab_subscribe", "start_time, end_time, user_id", $sql_Where);
			while($reserveList = $db->fetch_row($_reserveList))
			{
				...
				{
					$table[$startTime][$i] = intval($reserveList[2]);
				}
			}
		}
